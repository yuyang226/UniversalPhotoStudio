...
public class ImageCache {
    private static final String TAG = "ImageCache"; //$NON-NLS-1$
                ...
                if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
                    ...
                    if (getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
                        try {
                            ...
                            Log.e(TAG, "initDiskCache - " + e); //$NON-NLS-1$
                        }
                    }
                }
            }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    Log.e(TAG, "addBitmapToCache - " + e); //$NON-NLS-1$
                } catch (Exception e) {
                    Log.e(TAG, "addBitmapToCache - " + e); //$NON-NLS-1$
                } finally {
                    try {
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    Log.e(TAG, "getBitmapFromDiskCache - " + e); //$NON-NLS-1$
                } finally {
                    try {
                        if (inputStream != null) {
                            inputStream.close();
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
                try {
                    ...
                    Log.e(TAG, "clearCache - " + e); //$NON-NLS-1$
                }
                mDiskLruCache = null;
                initDiskCache();
            }
    ...
    public static class ImageCacheParams {
        ...
        public void setMemCacheSizePercent(Context context, float percent) {
            if (percent < 0.05f || percent > 0.8f) {
                throw new IllegalArgumentException("setMemCacheSizePercent - percent must be " //$NON-NLS-1$
                        + "between 0.05 and 0.8 (inclusive)"); //$NON-NLS-1$
            }
    ...
    public static String hashKeyForDisk(String key) {
        String cacheKey;
        try {
            final MessageDigest mDigest = MessageDigest.getInstance("MD5"); //$NON-NLS-1$
    ...
    public static File getExternalCacheDir(Context context) {
        ...
        final String cacheDir = "/Android/data/" + context.getPackageName() + "/cache/";  //$NON-NLS-1$//$NON-NLS-2$
